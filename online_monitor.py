...
PING_HOST = "8.8.8.8"
WM_WTSSESSION_CHANGE = 0x02B1  # Windows-defined constant, value = 689
...
class LockMonitor(threading.Thread):
    ...
    def stop(self):
        self.running = False
        win32gui.PostMessage(self.hwnd, win32ts.WM_QUIT, 0, 0)
...
def add_to_startup():
    ...
    with winreg.OpenKey(key, key_path, 0, winreg.KEY_WRITE) as reg:
        winreg.SetValueEx(reg, app_name, 0, winreg.REG_SZ, path)
...
if __name__ == "__main__":
    ...
    signal.signal(signal.SIGBREAK, handle_exit)  # Handles Ctrl+Break / console close

    app.start()
    print("App started. Press Ctrl+C to exit.")